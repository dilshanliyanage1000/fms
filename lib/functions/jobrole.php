...
function getallJobroles()
{
    ...
    $view_sql = "SELECT * FROM emp_jobrole_tbl ORDER BY jobrole_id DESC;";
    ...
    if ($nor > 0) {
        ...
        while ($rec = mysqli_fetch_assoc($view_result)) {
            ...
            echo ("<td><button id=" . $rec['jobrole_id'] . " class='btn btn-primary btn-sm' data-toggle='modal' data-target='#editModal'>Edit</button></td>");
            echo ("<td><button id=" . $rec['jobrole_id'] . " class='btn btn-danger btn-sm'>Delete</button></td>");
