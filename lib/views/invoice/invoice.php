...
?>
    ...
    <div class="col-md-12">
        <div class="content">
            <div class="jumbotron">
                <h1 class="display-5" style="text-align: center;"><i class="fas fa-file-invoice fa-1x"></i>&nbsp;&nbsp;Create Invoice (Bill)</h1>
                <hr>
                <br>
                <form id="invoice">
                    ...
                    <div class="row">
                        <div class="col-md-10">
                            <div class="form-group">
                                <label>Search products for Invoice:</label>
                                <input type="text" name="search_prod" id="search_prod" class="form-control" placeholder="Product Name (නිෂ්පාදන නාමය) / Manufacture Code (නිෂ්පාදන කේතය)">
                            </div>
                        </div>
                        <div class="col-md-10">
                            <div class="form-group" style="width: 100%;">
                                <div class="container" id="prodInfo" style="width: 100%; border: 3px #eee solid; overflow: auto; white-space: nowrap; background-color: #eee; display: none;">
                                </div>
                            </div>
                        </div>
                    </div>
                    <hr style="width:100%; margin-left:0; border: 1px solid #dbdbdb;">
                    <br>
                    <h5 class="card-title"><i class="fas fa-boxes"></i>&nbsp;&nbsp;Invoice Product List</h5>
                    <hr class="my-4">
                    <br>
                    <div class="row" style="margin-bottom:20px;">
                        <table id="invoice_list" class="table table-hover table-striped table-inverse table-responsive table-bordered" style="margin-left:10px; margin-top:10px; width:100%; font-size: 16px;">
    ...
    <script>
        $(document).ready(function() {
            ...
            $("#genInvoice_btn").click(function() {
                ...
                $("#invoice_list tr").each(function(row, tr) {
                    TableData[row] = {
                    ...
                    if (ProdListTbl == '[]') {
                        ...
                        if (method == 'cash') {
                            ...
                            if (transaction_receipt_no == "") {
                                ...
                                $.when(
                                    window.open("./invoice_pdf.php?firstname=" + firstname + "&lastname=" + lastname + "&email=" + email + "&code_phoneone=" + code_phoneone + "&telone=" + telone + "&code_phonetwo=" + code_phonetwo + "&teltwo=" + teltwo + "&houseno=" + houseno + "&streetone=" + streetone + "&streettwo=" + streettwo + "&city=" + city + "&postalcode=" + postalcode + "&cashier_id=" + logged_user + "&loggedusername=" + logged_user_name + "&paymentmethod=" + paymentmethod + "&transaction_receipt_no=" + transaction_receipt_no + "&ProdListTbl=" + ProdListTbl + "&master_total=" + master_total + "&discount=" + discountperc + "&finaltotal=" + finalprice + "&specialnotes=" + specialnotes + "&/", "_blank")
                                ).then(function() {
                                    setTimeout(() => {
                                        window.location.href = './invoice_list.php';
                                    }, 2550);
                    ...
                    $.get("../../route/customer/getsingleCus.php", {
                        ...
                        if (ProdListTbl == '[]') {
                            ...
                            if (method == 'cash') {
                                ...
                                $.when(
                                    window.open("./invoice_selected_pdf.php?selected_cus_id=" + selected_cus_id + "&cashier_id=" + logged_user + "&loggedusername=" + logged_user_name + "&paymentmethod=" + paymentmethod + "&ProdListTbl=" + ProdListTbl + "&master_total=" + master_total + "&discount=" + discountperc + "&finaltotal=" + finalprice + "&specialnotes=" + specialnotes + "&/", "_blank")
                                ).then(function() {
                                    setTimeout(() => {
                                        window.location.href = './invoice_list.php';
                                    }, 2550);
                                ...
                                if (transaction_receipt_no == "") {
                                    ...
                                    $.when(
                                        window.open("./invoice_selected_pdf.php?selected_cus_id=" + selected_cus_id + "&cashier_id=" + logged_user + "&loggedusername=" + logged_user_name + "&paymentmethod=" + paymentmethod + "&transactionreceipt=" + transaction_receipt_no + "&ProdListTbl=" + ProdListTbl + "&master_total=" + master_total + "&discount=" + discountperc + "&finaltotal=" + finalprice + "&specialnotes=" + specialnotes + "&/", "_blank")
                                    ).then(function() {
                                        setTimeout(() => {
                                            window.location.href = './invoice_list.php';
                                        }, 2550);
            ...
            $("#invoice_list tbody").on("click", ".btn-del", function() {
            ...
            $("#search_prod").keyup(function() {
                var searchVal = $(this).val();
                $.get("../../route/invoice/searchProduct.php", {
                    data: searchVal
                }, function(data) {
                    if (searchVal.length < 1) {
                        $("#prodInfo").hide();
                        ...
                        $(".btn-qtyadd").click(function() {
                            ...
                            $.get("../../route/invoice/getProductStockQty.php", {
                                id: productID
                            }, function(data) {
                        ...
                        $(".btn-selectprod").click(function() {
                            ...
                            $.get("../../route/invoice/fetchProdResult.php", {
                                id: prodID
                            }, function(data) {
                                var jdata = jQuery.parseJSON(data);
                                ...
                                if (thistable.includes(prodid)) {
                                    swal("This Product is already included!", "Please remove the relevant row and re-enter the appropriate quantity", "warning");
                                } else {
                                    var unprice = parseFloat(produnitprice);

                                    var prdqty = parseFloat($("#TXT" + prodID).val());

                                    var price = parseFloat(unprice * prdqty);

                                    $("#invoice_list tbody").append('<tr><td>' + prodid + '</td><td>' + prodcode + '</td><td>' + prodname + '&nbsp;(' + prodmotorcapacity + ')</td><td>Rs. ' + produnitprice + '.00</td><td>' + prdqty + '</td><td class="total-price">Rs. ' + price + '.00</td><td><button class="btn btn-danger btn-del" type="button"><i class="fas fa-trash"></i></button></td></tr>');
