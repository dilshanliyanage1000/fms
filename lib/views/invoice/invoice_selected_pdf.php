...
$obj_pdf->SetTitle("UDAYA INDUSTRIES : INVOICE");
...
foreach ($tbl as $row) {
  ...
  $invItems = invoiceItems($regInvoice, $prodID, $unpriceonly, $qty, $totpriceonly);

  $invDesc = $invDesc .= '[' . $itemcode . '] ' . $prodname . ' / ';

  $tblContent .= '<tr style="text-align:center;">
                    <td>#' . $c . '</td>
                    <td style="text-align:left;">' . $prodname . '</td>
                    <td style="text-align:right;">' . $unpricefinal . '</td>
                    <td style="text-align:center;">' . $qty . '</td>
                    <td style="text-align:right;">' . $totpricefinal . '</td>
                  </tr>';
...
$pathsend = '../../docs/invoice/' . $pdfname . '.pdf';
...
file_put_contents('../../docs/invoice/' . $pdfname . '.pdf', $pdf_string);
...
$sendPath = '../../docs/advice_of_dispatch/' . $nameofPDF . '.pdf';

$conn = Connection();

$updateQuery = "UPDATE invoice_tbl SET aod_pdf_path = '$sendPath' WHERE inv_id = '$regInvoice';";
...
file_put_contents('../../docs/advice_of_dispatch/' . $nameofPDF . '.pdf', $pdfnamestring);
...
$pathtoDB = '../../docs/goods_issued_order/' . $PDFText . '.pdf';

$conn = Connection();

$newQuery = "UPDATE invoice_tbl SET gio_pdf_path = '$pathtoDB' WHERE inv_id = '$regInvoice';";
...
file_put_contents('../../docs/goods_issued_order/' . $PDFText . '.pdf', $stringofPDF);
